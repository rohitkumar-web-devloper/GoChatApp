import { View, Text, SafeAreaView, StatusBar, TouchableOpacity, TextInput, Dimensions, useColorScheme, ScrollView, ToastAndroid, Alert } from 'react-native';
      ...
      <ScrollView bounces={false}>
        <View style={{ height: height, backgroundColor: theme == "dark" ? Black : White }}>
          <Formik
            ...
            onSubmit={(values) => {
              axios.post(`${Main_Base}/create/register-with-email`, { fullName: values.fullName, email: values.email, password: values.password })
                .then(async (response) => {
            ...
            {({ handleChange, handleBlur, handleSubmit, values, touched, isValid, errors }) => (
              <View style={{ flex: 1, padding: scale(10), flexDirection: "column", justifyContent: "space-between", }}>
                <View>
                  ...
                  <View>
                    <View>
                      <TextInput style={[MainStyle.input, { color: theme == "dark" ? White : Black, borderColor: errors.fullName && touched.fullName ? "red" : LGray }]} placeholderTextColor={DGray}
                      ...
                      {errors.fullName && touched.fullName ? <Text style={{ color: "red", fontSize: 10, textAlign: "right", position: 'absolute', top: 70, right: 10, fontWeight: 400, }}>{errors.fullName}</Text> : null}
                    </View>
                    <View>
                      ...
                      {errors.email && touched.email ? <Text style={{ color: "red", fontSize: 10, textAlign: "right", position: 'absolute', top: 70, right: 10, fontWeight: 400, }}>{errors.email}</Text> : null}
                    </View>
                    <View>
                      <TextInput style={[MainStyle.input, { color: theme == "dark" ? White : Black, }]} placeholderTextColor={DGray}
                      ...
                      {errors.password && touched.password ? <Text style={{ color: "red", fontSize: 10, textAlign: "right", position: 'absolute', top: 70, right: 10, fontWeight: 400, }}>{errors.password}</Text> : null}
                    </View>
                    <View>
                      <TextInput style={[MainStyle.input, { color: theme == "dark" ? White : Black, }]} placeholderTextColor={DGray}
                      ...
                      {errors.confirmPassword && touched.confirmPassword ? <Text style={{ color: "red", fontSize: 10, textAlign: "right", position: 'absolute', top: 70, right: 10, fontWeight: 400, }}>{errors.confirmPassword}</Text> : null}
                    </View>
                  </View>
                </View>
